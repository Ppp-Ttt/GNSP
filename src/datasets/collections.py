...
class LabelOrderSampler(Sampler):
    def __init__(self, data_source):
        self.data_source = data_source

    def __iter__(self):
        labels_and_indices = [(label, index) for index, (data, label) in enumerate(self.data_source)]
        labels_and_indices.sort(key=lambda x: x[0])
        return iter([index for label, index in labels_and_indices])

    def __len__(self):
        return len(self.data_source)

class TaskOrderSampler(Sampler):
    def __init__(self, data_source, increment_size):
        self.data_source = data_source
        self.increment_size = increment_size

    def __iter__(self):
        labels_and_indices = [(label, index) for index, (data, label) in enumerate(self.data_source)]
        labels_and_indices.sort(key=lambda x: x[0]//self.increment_size)
        return iter([index for label, index in labels_and_indices])
...
class ClassificationDataset:
    ...
    def build_dataloader(self):
        if self.sampler == 'TaskOrderSampler':
            self.train_sampler = TaskOrderSampler(self.train_dataset, 10)
            self.test_sampler = TaskOrderSampler(self.test_dataset, 10)
        elif self.sampler == 'LabelOrderSampler':
            self.train_sampler = LabelOrderSampler(self.train_dataset)
            self.test_sampler = LabelOrderSampler(self.test_dataset)
        else:
    ...
    def pick_prompt(self, classname):
        if self.prompts is not None:
            return random.choice(self.prompts[classname])
        return self.template(classname)

    def get_prompts(self, path):
        self.prompts = pickle.load(open(path, "rb"))
...
class Flowers(ClassificationDataset):
    def __init__(self, *args, **kwargs):
        ...
        # self.train_dataset._labels = [label-1 for label in self.train_dataset._labels]
        # self.test_dataset._labels = [label-1 for label in self.test_dataset._labels]
