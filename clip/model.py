...
class ResidualAttentionBlock(nn.Module):
    ...
    def get_hidden_state(self, x: torch.Tensor):
        x = x + self.attention(self.ln_1(x))
        x0 = self.ln_2(x)
        x1 = self.mlp.gelu(self.mlp.c_fc(x0))
        x2 = self.mlp.c_proj(x1)
        x_out = x2 + x
        return x0, x1, x_out
...
class Transformer(nn.Module):
    ...
    def get_image_matrix_ffn(self, x: torch.Tensor):
        ffn_x0 = []
        ffn_x1 = []
        for block in self.resblocks:
            x0, x1 , x= block.get_hidden_state(x)
            ffn_x0.append(x0)
            ffn_x1.append(x1)

        ffn_x0 = torch.stack(ffn_x0) # shape: (block_num, 197, batch_size, 768)
        ffn_x1 = torch.stack(ffn_x1)

        return ffn_x0, ffn_x1, x
...
class VisualTransformer(nn.Module):
    ...
    def get_image_matrix(self, x: torch.Tensor):
        ...
        ffn_x0, ffn_x1, x= self.transformer.get_image_matrix_ffn(x)
        x = x.permute(1, 0, 2)  # LND -> NLD
        ffn_x0 = ffn_x0.permute(0,2,1,3)
        ffn_x1 = ffn_x1.permute(0,2,1,3)
...
class CLIP(nn.Module):
    ...
    def get_image_matrix(self, image):
        with torch.no_grad():
            return self.visual.get_image_matrix(image.type(self.dtype))
...
def build_model(state_dict: dict):
    ...
    if vit:
        ...
        # print("building ResNet CLIP model")
